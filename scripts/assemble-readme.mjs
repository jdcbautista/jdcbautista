...
// Markdown emphasis → HTML (so it renders inside centered <p> blocks).
const emph = (s) => s.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>");
// SB7 description: the visitor/team is the hero, Julius is the guide. Held to a
// readable width via a single-cell table so centered prose doesn't sprawl.
const descBlock = `<table align="center" width="720"><tr><td align="center">\n\n${p.description
  .map((par) => emph(par))
  .join("\n\n")}\n\n</td></tr></table>`;
